...
@MainActor
@Observable class PendulumRunner: SavableEnvironmentRunner {
    var snapshot: PendulumSnapshot?
    var episodeCount = 1
    var currentStep = 0
    ...
    var combinedBestReward: Double {
        let newBest = episodeMetrics.map { $0.reward }.max() ?? -2000
        return max(loadedBestReward, newBest)
    }
    
    static var environmentType: EnvironmentType { .pendulum }
    static var displayName: String { "Pendulum" }
    static var algorithmName: String { "SAC" }
    static var icon: String { "circle.circle" }
    ...
    func setupEnvironment() {
        ...
        guard let madeEnv = Gymnazo.make(
            ...
            print("Failed to create Pendulum environment")
            return
        }
    ...
    private func updateSnapshot() {
        if let pendulum = self.env?.unwrapped as? Pendulum {
            self.snapshot = pendulum.currentSnapshot
            if let snap = snapshot {
                self.theta = snap.theta
                self.thetaDot = snap.thetaDot
            }
        }
    }
    ...
    func saveAgent(name: String) throws {
        ...
        let saved = try AgentStorage.shared.savePendulumAgent(
            name: name,
            actor: agent.actor,
    ...
    func updateAgent(id: UUID, name: String) throws {
        ...
        try AgentStorage.shared.updatePendulumAgent(
            id: id,
            newName: name,
            actor: agent.actor,
            qEnsemble: agent.qEnsemble,
    ...
    func loadAgent(from savedAgent: SavedAgent) throws {
        guard savedAgent.environmentType == .pendulum else {
        ...
        let weightsDict = try AgentStorage.shared.loadPendulumWeights(for: savedAgent)
    ...
    private func trainingLoop() async {
        ...
        if totalSteps < warmupSteps {
            ...
            for i in 0..<samplesToCollect {
                let (newKey, actionKey) = MLX.split(key: rngKey)
                rngKey = newKey
                let range: Range<Float> = PendulumSAC.actionLow..<PendulumSAC.actionHigh
                let action = MLX.uniform(range, [PendulumSAC.actionCount], key: actionKey)
        ...
        while isTraining && episodesCompleted < episodesPerRun {
            ...
            await MainActor.run {
                ...
                let metrics = EpisodeMetrics(
                    ...
                    steps: finalSteps,
                    success: episodeRewardLocal > -500,
                    averageTDError: 0,
                    averageLoss: nil,
                    averageMaxQ: 0,
                    epsilon: 0,
