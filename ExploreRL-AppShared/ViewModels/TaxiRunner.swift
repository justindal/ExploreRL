...
@MainActor
@Observable class TaxiRunner: SavableEnvironmentRunner {
    var snapshot: TaxiRenderSnapshot?
    var episodeCount = 1
    var currentStep = 0
    ...
    static var environmentType: EnvironmentType { .taxi }
    static var displayName: String { "Taxi" }
    static var algorithmName: String { "Q-Learning / SARSA" }
    static var icon: String { "car.fill" }
    static var accentColor: Color { .yellow }
    ...
    var ficklePassenger: Bool = false
    ...
    static let stateSize = 500
    static let actionSize = 6
    ...
    func setupEnvironment() {
        ...
        guard let madeEnv = Gymnazo.make(
            ...
            print("Failed to create Taxi environment")
            return
        }
    ...
    private func updateSnapshot() {
        if let taxi = self.env?.unwrapped as? Taxi {
            self.snapshot = taxi.currentSnapshot
        }
    }
    ...
    func saveAgent(name: String) throws {
        ...
        let saved = try AgentStorage.shared.saveTaxiAgent(
            name: name,
            qTable: agent.qTable,
    ...
    func updateAgent(id: UUID, name: String) throws {
        ...
        try AgentStorage.shared.updateTaxiAgent(
            id: id,
            newName: name,
            qTable: agent.qTable,
            episodesTrained: totalEpisodesTrained,
    ...
    func loadAgent(from savedAgent: SavedAgent) throws {
        guard savedAgent.environmentType == .taxi else {
    ...
    private func runTrainingLoop() async {
        ...
        for episode in episodeCount..<targetEpisodes {
            ...
            // Success is when the passenger is delivered (episode terminates with positive final reward)
            let success = terminated && episodeRewardLocal > 0
