//
//  BlackjackConfigurationView.swift
//

import SwiftUI

struct BlackjackConfigurationView: View {
    ...
    private var environmentSection: some View {
        VStack(alignment: .leading) {
            ...
            Toggle("Natural Blackjack Bonus", isOn: $runner.natural)
                .onChange(of: runner.natural) { _, _ in
                    guard !runner.isLoadingAgent else { return }
                    runner.reset()
                }
            
            Text("Give +1.5 reward for natural blackjack (21 on first two cards)")
                .font(.caption)
                .foregroundStyle(.secondary)
            
            Toggle("Sutton & Barto Rules", isOn: $runner.sab)
                .onChange(of: runner.sab) { _, _ in
                    guard !runner.isLoadingAgent else { return }
                    runner.reset()
                }
            
            Text("Follow exact rules from Sutton & Barto textbook")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
        .disabled(runner.isTraining)
