...
struct LunarLanderContinuousView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showLoadSheet) {
            LoadAgentSheet(environmentType: .lunarLanderContinuous) { agent in
            ...
            ScrollView {
                ...
                case .settings:
                    LunarLanderContinuousConfigurationView(runner: runner)
                        .padding()
                case .charts:
                    LunarLanderContinuousChartsView(runner: runner)
                        .padding()
                }
            }
        }
            ...
            if runner.renderEnabled {
                if let snapshot = runner.snapshot {
                    LunarLanderContinuousViewAdapter(snapshot: snapshot)
                        .aspectRatio(600.0 / 400.0, contentMode: .fit)
                        .frame(maxWidth: 700, maxHeight: 467)
                        .cornerRadius(12)
                        .shadow(radius: 5)
                } else {
                    Rectangle()
                ...
                ScrollView {
                    LunarLanderContinuousChartsView(
                        runner: runner,
        ...
        VStack(spacing: 12) {
            ...
            HStack(spacing: 16) {
                if runner.isTraining {
                    ...
                    .buttonStyle(TrainingButtonStyle(color: .teal))
                    .accessibilityLabel(runner.canResume ? "Resume Training" : "Start Training")
                    .accessibilityHint("Begins training the Lunar Lander Continuous agent")
