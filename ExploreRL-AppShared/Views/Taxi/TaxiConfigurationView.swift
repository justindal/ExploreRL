//
//  TaxiConfigurationView.swift
//

import SwiftUI

struct TaxiConfigurationView: View {
    ...
    private var environmentSection: some View {
        VStack(alignment: .leading) {
            ...
            Toggle("Rainy Weather", isOn: $runner.isRainy)
                .onChange(of: runner.isRainy) { _, _ in
                    guard !runner.isLoadingAgent else { return }
                    runner.reset()
                }
            
            Text("Movement has 80% intended direction, 10% slip left/right")
                .font(.caption)
                .foregroundStyle(.secondary)
            
            Toggle("Fickle Passenger", isOn: $runner.ficklePassenger)
                .onChange(of: runner.ficklePassenger) { _, _ in
                    guard !runner.isLoadingAgent else { return }
                    runner.reset()
                }
            
            Text("30% chance passenger changes destination after pickup")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
        .disabled(runner.isTraining)
